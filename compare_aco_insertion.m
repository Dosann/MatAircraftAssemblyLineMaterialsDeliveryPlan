...
cursor=exec(conn,'select id,N,density,crowdness,aco_feasib,insertion_feasib from cases where m_ub<=10');
cases=fetch(cursor);
cases=cases.Data;
...
insertion_solvable=cases(:,5)==0 & cases(:,6)==1;
insertion_solvable_count=sum(insertion_solvable)
...
cc=join(caseid_str,',');
cursor=exec(conn,sprintf('select id,result_aco,result_insertion from cases where id in %s',['(',cc{1},')']));
cases=fetch(cursor);
cases=cases.Data;
...
for i=1:size(cases,1);
    ...
    result_insertion=jsondecode(cases{i,3});
    objs(i,1)=cases{i,1};
    objs(i,2)=result_aco.bestObj_hist;
    objs(i,3)=result_insertion.obj;
end
objs;
