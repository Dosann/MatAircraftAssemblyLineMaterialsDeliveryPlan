...
for N=30:30:300
    for m_ub=5:1:10
        for density_cate=1:5
            cursor=exec(conn,sprintf('select id from cases where N=%d and m_ub=%d and density_cate=%d',N,m_ub,density_cate));
            cases=fetch(cursor);cases=cases.Data;ids=cell2mat(cases);
            if strcmp(ids,'No Data')
                continue
            end
            id_selected=ids(ceil(rand(1)*length(ids)))
            cursor=exec(conn,sprintf('select id,data from cases where id=%d',id_selected));
            cases=fetch(cursor);cases=cases.Data;
            cursor=exec(conn,sprintf('insert into SA_aco(id,data) values(%d,''%s'')',cases{1,1},cases{1,2}));
        end
    end
end
